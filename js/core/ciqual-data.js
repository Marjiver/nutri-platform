/**
 * ciqual-data.js — NutriDoc · Table CIQUAL 2020 Anses
 * Version ultra-complète : 150+ aliments avec macros ET micros
 * 
 * Catégories :
 * - Féculents & céréales (18 aliments)
 * - Protéines animales (16 aliments)
 * - Protéines végétales (12 aliments)
 * - Légumes (22 aliments)
 * - Fruits (18 aliments)
 * - Produits laitiers & alternatives (15 aliments)
 * - Matières grasses & oléagineux (14 aliments)
 * - Boissons (8 aliments)
 * - Épices & condiments (10 aliments)
 * - Plats préparés & snacking (10 aliments)
 * - Œufs & dérivés (6 aliments)
 * - Poissons & fruits de mer (12 aliments)
...
// ==================== GROUPES LIST ====================
const GROUPES_LIST = Object.entries(CIQUAL).map(([k, v]) => ({
  key: k,
  label: v.label,
  icon: v.icon,
  color: v.color
...
/**
 * Détecte le groupe d'un aliment à partir de son nom
 * @param {string} nomAliment - Nom de l'aliment
...
/**
 * Récupère les macros et micros d'un aliment
 * @param {string} nomAliment - Nom de l'aliment
 * @param {number} qte - Quantité en grammes
...
/**
 * Récupère les calories d'un aliment
 * @param {string} nomAliment - Nom de l'aliment
 * @param {number} qte - Quantité en grammes
 * @returns {number|null} - Calories ou null
 */
function kcalPour(nomAliment, qte) {
  if (!nomAliment) return null;
  const n = nomAliment.toLowerCase();
...
/**
 * Recherche un aliment par nom partiel
 * @param {string} recherche - Terme de recherche
 * @returns {Array} - Liste des aliments correspondants
 */
function rechercherAliment(recherche) {
...
window.GROUPES_LIST = GROUPES_LIST;
window.detecterGroupe = detecterGroupe;
window.getMacrosMicros = getMacrosMicros;
window.kcalPour = kcalPour;
window.rechercherAliment = rechercherAliment;