// ============================================================
//  Supabase Edge Function : generate-plan (version OpenAI + CIQUAL)
...
import { CIQUAL, detecterGroupe, getMacrosMicros, kcalPour } from "./ciqual-data.ts";
...
function getCiqualSummary(): string {
  ...
  for (const [key, group] of Object.entries(CIQUAL)) {
    // On prend les 8 premiers aliments de chaque catégorie
    const aliments = group.equivalents.slice(0, 8).map(e => e.aliment).join(", ");
    summary.push(`**${group.label}** : ${aliments}...`);
  }
  return summary.join("\n");
}

// Optionnel : enrichir le plan généré avec les données CIQUAL (ex. ajouter les calories estimées)
function enrichirAvecCiqual(plan: any): any {
  ...
  for (const jour of plan.semaine) {
    for (const repas of ["matin", "collation_matin", "midi", "collation_soir", "soir"]) {
      ...
      if (item?.description) {
        // Cherche un aliment correspondant dans CIQUAL
        const mots = item.description.toLowerCase().split(/\s+/);
        let found = null;
        for (const grp of Object.values(CIQUAL)) {
          found = grp.equivalents.find(e => 
            mots.some(m => e.aliment.toLowerCase().includes(m) && m.length > 3)
          );
          if (found) break;
        }
        if (found) {
...
Adapte précisément aux restrictions alimentaires, allergies et objectif. Utilise UNIQUEMENT des noms d'aliments présents dans la liste CIQUAL. Réponds UNIQUEMENT avec le JSON, sans texte autour.`;
    ...
    const completion = await openai.chat.completions.create({
      model: "gpt-4o-mini", // ou "gpt-4.1" pour plus de qualité
    ...
    // Nettoyage des éventuels markdown
